...
第 1～15 课已经完成。第 14 课在同步会话问答前拆分复合问题，并为每个子问题选择知识库作用域；第 15 课在同一作用域内并行执行向量和关键词召回，再用 RRF、精排和证据门槛选择上下文；第 16 课开始为模型调用增加档位选择和失败切换。
...
当前普通测试应有十九个全部通过：历史业务回归、结构化文档测试、第 12 课流程结构测试和第 16 课模型路由状态测试。
