...
为了避免把不同层次混在一起，本课只处理“父问题如何变成若干个子问题，以及每个子问题允许查哪里”。它还没有进入 Chunk 级别的向量/关键词召回。

```text
父问题
  → 会话记忆：补回历史上下文
  → 子问题拆分：一个父问题变成一个或多个子问题
  → IntentPlan：为每个子问题决定作用域
       ├─ 唯一知识库：允许继续检索
       ├─ 没有候选：knowledgeBaseIds 为空，表示暂时不限定库，回落全库
       └─ 多个同分候选：无法安全决定，返回澄清，不执行检索
  → 第 15 课的每个子问题再进入多通道检索
```

“命中两个库”有两种完全不同的情况：

- 如果是一个父问题拆成两个子问题，例如“年假有几天，并且报销需要什么材料”，每个子问题各自只有一个最佳库，因此会分别检索两次，最后聚合两个答案。
- 如果是同一个子问题同时对两个库得到相同最高分，例如两个库都叫“年假”，系统无法判断应该查哪一个，因此 `clarificationRequired=true`，在 `IntentRoutingService` 中提前返回，根本不调用 `answerFromDatabase`。

`knowledgeBaseIds` 使用列表是为了表达“当前子问题允许访问的作用域”，不是说列表里有几个库就一定要执行几次检索。正常执行时列表通常只有一个 ID；空列表是一个明确约定，表示低置信度时暂时不限制知识库，交给后续检索做全库搜索。请求中的 `knowledgeBaseId` 则是调用方已经明确指定的库，会优先成为计划的作用域。

函数名称也有一层翻译：`plan` 是“做出一份计划”，`route` 是“按照计划把问题送到正确路径”。所以 `IntentPlan` 保存的是规划结果，`IntentRoutingService` 消费这些结果并执行路由；它们有关联，但不是同一个动作。
